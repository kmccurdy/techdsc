...
- **Establish 'data unions' for collective representation and redress**: Support mechanisms for platform users, as producers of data, to have a voice in how that data is used
- **Fund the Universal Basic Dividend through tech profits**: Require firms to dedicate some percentage of their shares to a public trust
- **Pursue different models of platform ownership**: There are several options for rethinking platform models, from structural support for platform co-operatives, to category shifts for existing platforms (e.g. regulating them as utilities), to outright collectivization. Democratic debate and exploration of these various alternatives must be supported.
